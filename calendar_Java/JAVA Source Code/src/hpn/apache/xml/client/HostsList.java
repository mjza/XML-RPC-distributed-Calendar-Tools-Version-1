...
public class HostsList implements CalendarNetwork
{
	...
	protected static void initHostList(int port) //it must call locally in the package by hpn.apache.xml.client.HpnXmlRpcClient Contractor function to register local host and initiate the list
	{//its modifier must be protected to prevent RPC calls
		if(hostsAddresses.isEmpty())
			hostsAddresses.add(new HostUrl(port)); //Set the first server to the local server for internal requests
	}
	@Override
	...
	protected static boolean removeHost(HostUrl hostUrl) 
	{
		//this function will call by current client (locally)
		try{
			//check for iterated(repited) host
		...
		{
			System.err.println("Rupturing an old host has crached, maybe entered URL or port has a problem.");
			System.err.println(e.getMessage());
		}
		return false;//if the adding process fail in any cases, this false value will show this failure
	...
	protected static HostUrl getFirstHostUrl() throws ArrayIndexOutOfBoundsException
	{
