...
public final class HpnXmlRpcClient extends HpnClientFunctionality {
	...
	private  boolean setLocalHost() 
	{
		try 
		{
			config.setServerURL(new URL(HostsList.getFirstHostUrl().getFullUrl())); // http://168.12.2.14:8080/
	...
	protected void sendJoinRequest() 
	{
		...
		System.out.println("-------------------------------------------");
		System.out.print("Please enter the IPv4 address of a know host that is a member of the calendar network currently : ");
	...
	protected void addAppointment() 
	{
		...
		} catch (MalformedURLException e) { //for hostUrl.getFullUrl()
			System.out.println(e.getMessage());
		}
	}	
	@Override
	protected void removeAppointment() {
		...
		while(true)
		{
			...
			if(response == 'n' || response == 'N')
				...
				System.out.print("The character that you have entered ['"+response+"'] is not correct. You can just enter a character from the set {'n','N','y','Y'}.");
		}
		...
		{	
			if(this.setLocalHost())
			{
                ...
                do
                {
                    try
                    {		
						result = (boolean)  host.execute("Calendar.removeAppointment", params); //make new appointment on local machine
						break;
                    }
                    catch (Exception e)
	                {
	...
	protected void modifyAppointment() {
        ...
        while (true)
        {
            ...
            {
                System.out.println("So please enter a new header for this appointment.");
        ...
        while (true)
        {
            ...
            {
                System.out.println("So please enter a new comment for this appointment.");
		...
		{	//it acts on all servers, an iterator on HostList will give the next host
			if(this.setLocalHost())
			{
                ...
                do
                {
                    try
                    {
                    	result = (boolean)  host.execute("Calendar.modifyAppointment", params); //make new appointment on local machine
                    	break;
                    }
                    catch (Exception e)
                    {
					...
					while(hostUrl!=null)
					{
						if(this.setDestinationHost(hostUrl))
						{
                             ...
                             do
                             {
                                 try
                                 {
                                	 result = (boolean) host.execute("Calendar.modifyAppointment", params); //make appointment on next host
                                	 break;
                                 }
                                  catch (Exception e)
                                 {
